    ...
    <Section id="about" className="pt-10 lg:pt-0 pb-10 lg:pb-0">
      ...
      <div className="grid lg:grid-cols-2 gap-16 items-start">
        ...
        >
          ...
          <div className="relative bg-zinc-950 border border-blue-500/30 rounded-2xl p-6 lg:p-10 overflow-hidden">
            <div className="relative w-48 h-48 mx-auto mb-8">
              ...
              <div className="absolute inset-4 rounded-full overflow-hidden bg-zinc-950 border border-zinc-800 shadow-inner">
                <img
                  src="/myself.jpg"
                  alt="System Avatar"
              ...
              <div className="absolute bottom-4 right-4 w-4 h-4 bg-green-500 rounded-full border-2 border-zinc-900 animate-pulse" />
            </div>
            ...
            <div className="font-mono text-sm space-y-3">
              ...
              <div className="flex justify-between items-center border-b border-zinc-800 pb-2">
                ...
                <span className="text-foreground uppercase">{PROFILE.location}</span>
              </div>
              <div className="flex justify-between items-center pb-2">
                <span className="text-zinc-500">STATUS</span>
                <span className="bg-success/10 text-success px-2 py-0.5 rounded text-xs border border-success/20">{PROFILE.status}</span>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <div className="space-y-8">
          <motion.div
            className="bg-zinc-950 rounded-lg border border-zinc-800 font-mono text-sm shadow-xl overflow-hidden group hover:border-blue-500/30 transition-colors"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ delay: 0.2 }}
          >
            <div className="bg-zinc-800 px-4 py-2 flex items-center gap-2 border-b border-zinc-800">
          ...
          >
            {PROFILE.stats.map((stat, i) => {
              ...
              return (
                <div key={i} className="bg-zinc-950 p-4 rounded-lg border border-zinc-800 hover:border-blue-500/30 transition-all group">
                  <div className="flex items-center gap-2 mb-2">
                    <Icon className="w-4 h-4 text-blue-500" />
                    <span className="text-[10px] text-zinc-500 font-mono">{stat.label}</span>
                  </div>
                  <div className="text-xl font-bold text-foreground font-mono group-hover:text-blue-500 transition-colors">{stat.value} <span className="text-[10px] text-zinc-600">{stat.unit}</span></div>
                </div>
              );
            })}
          </motion.div>
